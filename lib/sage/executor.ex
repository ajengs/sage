defmodule Sage.Executor do
  ...
  defp maybe_execute_transaction({{name, operation}, state}, opts) do
    ...
    return = execute_transaction(operation, name, last_effect_or_error, effects_so_far, opts)
  ...
  defp execute_transaction({:run, transaction, _compensation, []}, name, latest_effect, effects_so_far, opts) do
    apply_transaction_fun(name, transaction, latest_effect, effects_so_far, opts)
  rescue
  ...
  defp execute_transaction({:run_async, transaction, _compensation, tx_opts}, name, latest_effect, effects_so_far, opts) do
  ...
  defp apply_transaction_fun(name, fun, latest_effect, effects_so_far, opts) do
  ...
  defp apply_function(fun, latest_effect, effects_so_far, opts) do
    case Function.info(fun, :arity) do
      {:arity, 2} -> apply(fun, [effects_so_far, opts])
      {:arity, 3} -> apply(fun, [latest_effect, effects_so_far, opts])
    end
  end
